...
test('POST /grpc/products.ProductsService/ViewProduct', { signal: AbortSignal.timeout(timeout) }, async () => {
  await runner
    .createScan({
      tests: ['sqli', 'business_constraint_bypass', 'proto_pollution', 'jwt', 'full_path_disclosure'],
    ...
    .run({
      ...
      body: '<binary grpc-web frame: ViewProductRequest { productName: "Wireless Mouse" }>',
      headers: {
